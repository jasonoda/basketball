...
export class Scene {
    ...
    buildScene(){
      ...
      this.reflectiveFloor.position.y=.135;
      ...
      this.court.traverse((object) => {
        ...
        if ( object.isMesh ){
          ...
          if(object.name==="glass" || object.name==="seatBackPurple2"){
          ...
          }else if( object.name==="courtRedOutside" || object.name==="courtLines" || object.name==="courtLines2" || object.name==="floor"){
            ...
            object.material.envMap = this.e.reflectionTexture;
            object.material.metalness = .125;
            object.material.roughness = .25;
          ...
          if(object.name==="adMobile1" || object.name==="adMobile2" || object.name==="adMobile3" || object.name==="adMobile4" || object.name==="adMobile5" ){
      ...
      for(var i=0; i<5; i++){
        ...
        this.ballShape = new CANNON.Sphere(.2);
        let ballBody = new CANNON.Body({ mass: 1, shape: this.ballShape, position: new CANNON.Vec3(1000, 5, 0), material: bouncyMaterial });
      ...
      this.floorBody.position.y = .12;
      this.world.addBody(this.floorBody);  this.floorBody.bType = "floor";
      ...
      this.boxShape = new CANNON.Box(new CANNON.Vec3(.84, .6, .1));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, 3.5, -.55), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "backboard";

      // bottom base

      this.boxShape = new CANNON.Box(new CANNON.Vec3(.55, .4, .74));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, .52, -3.5), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "wall";

      // top base

      this.boxShape = new CANNON.Box(new CANNON.Vec3(.3, 1, .54));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, 1.9, -3.5), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "wall";

      // hoop neck

      this.boxShape = new CANNON.Box(new CANNON.Vec3(.14, .1, .1));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, 3.06, -.35), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "wall";

      // wall back

      this.boxShape = new CANNON.Box(new CANNON.Vec3(14, 5, .1));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, 2.5, -10.5), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "wall";

      // wall right

      this.boxShape = new CANNON.Box(new CANNON.Vec3(.1, 5, 14));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(13.5, 2.5, 0), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "wall";
      ...
      this.boxShape = new CANNON.Box(new CANNON.Vec3(.12, .02, .02));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0, 3.15, .23), material: backboardMaterial });
      this.world.addBody(this.boxBody); this.boxBody.bType = "hoop";

      this.boxShape = new CANNON.Box(new CANNON.Vec3(.12, .02, .02));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(0.25, 3.15, -.01), material: backboardMaterial });
      ...
      this.boxShape = new CANNON.Box(new CANNON.Vec3(.12, .02, .02));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(-0.17, 3.15, .17), material: backboardMaterial });
      ...
      this.boxShape = new CANNON.Box(new CANNON.Vec3(.12, .02, .02));
      this.boxBody = new CANNON.Body({  mass: 0, shape: this.boxShape,  position: new CANNON.Vec3(-0.17, 3.15, -.20), material: backboardMaterial });
      this.boxBody.quaternion.setFromEuler(0, this.e.u.ca(45), 0);
      this.world.addBody(this.boxBody); this.boxBody.bType = "hoop";
      ...
      gsap.to(document.getElementById("scoreTotal"), { color: "white", duration: .8, yoyo: true, repeat: -1, ease: "sine.inOut" });
      gsap.to(document.getElementById("scoreTop"), { color: "white", duration: .8, yoyo: true, repeat: -1, ease: "sine.inOut" });
    ...
    update(){
      ...
      for(var i=0; i<this.balls.length; i++){
        ...
        if (Math.abs(this.ballPos.x - this.triggerPos.x) < 0.3 && Math.abs(this.ballPos.y - this.triggerPos.y) < 0.05 && Math.abs(this.ballPos.z - this.triggerPos.z) < 0.3 && this.balls[i].velocity.y < 0) { 
          ...
          this.spinFactor = 5;
          this.balls[i].angularVelocity.set( this.velocity.z * this.spinFactor, 0, -this.velocity.x * this.spinFactor);
      ...
      if(this.useLerp===true){
        ...
        this.e.camContY.rotation.y = this.e.u.lerp( this.e.camContY.rotation.y, this.targetLerp, .05);
      ...
      if(this.action==="start"){
        ...
        gsap.to(this.e.camContY.rotation, { y:this.e.u.ca(-15), duration: 12, yoyo: true, repeat: -1, ease: "sine.inOut" });
        ...
        gsap.to(this.e.camera.position, { z:11, duration: 2, ease: "expo.inOut" });
        gsap.to(this.e.camContY.rotation, { y:this.e.u.ca(90), duration: 2, ease: "expo.inOut" });

        this.e.s.p("whistle");

        gsap.to(this.startButton, { opacity:0, duration: .5, ease: "linear" });
        this.startButton.style.pointerEvents = "none"
        gsap.to(this.results, { opacity: 0, duration: .5, ease: "linear" });
        ...
        if(this.round===5){
          ...
          this.ballBody.position.set(worldPosition.x, worldPosition.y-1, worldPosition.z);
        ...
        const offsetVector = new THREE.Vector3(this.horNum * 0.02, 0, this.vertNum * 0.02);
        ...
        gsap.to(document.getElementById("meterDiv"), { opacity:0, duration: .5, ease: "linear" });
        gsap.to(document.getElementById("score"), { opacity:0, duration: .5, ease: "linear" });

        gsap.to(this.e.camera.position, { z:20, duration: 4, ease: "sine.inOut" });
        gsap.to(this.e.camContY.rotation, { y:this.e.u.ca(-15), duration: 4, ease: "sine.inOut" });

        document.getElementById("scoreTotal").innerHTML = this.basketsMade;

        gsap.to(this.results, { opacity: 1, duration: .5, ease: "linear" });
        gsap.to(this.startButton, { opacity: 1, duration: .5, ease: "linear" });
